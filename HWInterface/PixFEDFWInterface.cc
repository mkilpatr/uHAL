...
void PixFEDFWInterface::findPhases ()
{
    ...
    //cVecReg.push_back ( { "fe_ctrl_regs.fifo_config.overflow_value", 0x1806}); // 0x300c0 before - test on July 13 - Bora
...
void PixFEDFWInterface::prettyprintTBMFIFO (const std::vector<uint32_t>& pData )
{
    ...
    for ( auto& cWord : pData )
    {
        ...
        {
            //std::cout << cWord <<  std::endl;
            std::cout << " TBM Trailer: CH: " << ( (cWord >> 26) & 0x3f) << " ID: " << ( (cWord >> 21) & 0x1f) << " TBM T2: " << ( (cWord >> 12) & 0xff) << " Error Bits: "  <<  ( (cWord >> 8) & 0xf)  << " TBM_T1: " << (cWord & 0xff) << std::dec << std::endl;
        }
