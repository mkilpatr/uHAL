...
void SystemController::ConfigureHw( std::ostream& os )
{
    ...
    for (auto& cGLIB : fPixGLIBVector)
    {
        fGLIBInterface->ConfigureGLIB(cGLIB);
    }
}
...
void SystemController::parseHWxml( const std::string& pFilename, std::ostream& os )
{
    ...
    // Iterate the BeBoard Node
    for ( pugi::xml_node cBeBoardNode = doc.child( "HwDescription" ).child( "PixGLIB" ); cBeBoardNode; cBeBoardNode = cBeBoardNode.next_sibling("PixGLIB") )
    {
        ...
        cBeId = cBeBoardNode.attribute( "Id" ).as_int();
        //std::cout << +cBeId << std::endl;
        PixGLIB* cPixGLIB = new PixGLIB( cBeId );
        ...
        for ( pugi::xml_node cBeBoardRegNode = cBeBoardNode.child( "Register" ); cBeBoardRegNode; cBeBoardRegNode = cBeBoardRegNode.next_sibling("Register") )
        {
            std::string nodename = cBeBoardRegNode.name();
            if (nodename == "Register") os << BOLDCYAN << "|" << "----" << cBeBoardRegNode.name() << "  " << cBeBoardRegNode.first_attribute().name() << " :" << cBeBoardRegNode.attribute( "name" ).value() << " " << BOLDRED << convertAnyInt(cBeBoardRegNode.first_child().value()) << RESET << std:: endl;
            cPixGLIB->setReg( std::string( cBeBoardRegNode.attribute( "name" ).value() ), convertAnyInt( cBeBoardRegNode.first_child().value() ) );
        }

        fPixGLIBVector.push_back(cPixGLIB);

        PixGLIBFWInterface* cTmpFWInterface = new PixGLIBFWInterface(cBeBoardConnectionNode.attribute( "id" ).value(), cBeBoardConnectionNode.attribute( "uri" ).value(), cBeBoardConnectionNode.attribute("address_table").value() );
        /////////////////////////////////
        fFWMap_glib[cPixGLIB->getBeId()] = cTmpFWInterface;
    }

    // create the actual GLIB Interface using the map I previously filled
    fGLIBInterface = new PixGLIBInterface(fFWMap_glib);
