...
int main( int argc, char* argv[] ) {
	...
	HwInterface hw=manager.getDevice ( "GLIB.crate.slot_11" );   //this is to glib with the optical card
		...
		if( atoi(argv[3]) == 1){
			for (int i = 0; i < 8; i++){
				...
				string Marker_CHB ("Marker_Error_CHB_");
					...
					Marker_Value_CHB = (Marker_Value & 0xFFF0000) >> 16;		
					...
					hw3.getNode(Marker_Error).write( Marker_Value );
					ValWord < uint32_t > mem3 = hw3.getNode( Marker_Error ).read();
					hw3.dispatch();
					std::cout << Marker_Error << " = " << std::hex << mem3.value() << std::endl;
					...
					hw3.getNode(Marker_CHB).write( Marker_Value_CHB );
                			mem3 = hw3.getNode( Marker_CHB ).read();
                			hw3.dispatch();
                			std::cout << Marker_CHB << " = " << std::hex << mem3.value() << std::endl;
