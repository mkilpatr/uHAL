...
int main()
{
  ...
  std::cout << "  9. Test TBM/ROC resets on FIFO1" << std::endl;
  std::cout << " 10. Test DDR Timeouts" << std::endl;
  std::cout << " 11. Test DDR Timeouts with Marker Errors" << std::endl;
  std::cout << " 12. Test DDR Pixel Alive" << std::endl;
  std::cout << " " << std::endl;
  ...
  //}
	...
	std::cout << "Test second TBM reset" << std::endl;
	sleep(4);
	...
	std::cout << "Test second ROC reset" << std::endl;
	sleep(3);
	Test_Resets_FIFO1(1, 0);
	sleep(4);
  }
	...
	int loops;
	cin >> loops;
	Test_Timeouts(Delay1, Delay2, loops); //Sent events with two different timeouts and one event at a time
	...
	Marker_Error( Which_Chan, Marker_Type, Marker_Value, Marker_Rate); //Write to register to set the values of the Marker described in DDR.c or src/common/Marker_Multi_Chan.cxx
  	std::cout << "How many loops do you want to go through?" << std::cout;
	int loops;
	cin >> loops;
	Test_Timeouts( 32, 32, loops); //Send events and look at output
